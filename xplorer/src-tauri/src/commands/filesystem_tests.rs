mod tests {
    ...
    async fn test_list_directory() {
        ...
        let entries = list_directory(dir.path().to_string_lossy().into_owned(), true)
            .await
            .unwrap();
    ...
    async fn test_integration_flow() {
        ...
        assert!(copy_files(vec![f1_path.clone()], dest_dir.clone())
            .await
            .is_ok());
        assert!(
            rename_file(format!("{}/f1.txt", dest_dir), "f1_copy.txt".to_string())
                .await
                .is_ok()
        );
        ...
        // 4. 移動
        assert!(move_files(
            vec![format!("{}/f1_copy.txt", dest_dir)],
            target_dir.clone()
        )
        .await
        .is_ok());
    ...
    async fn test_e2e_real_directory() {
        ...
        assert!(copy_files(vec![file1.clone()], sub_dir.clone())
            .await
            .is_ok());
        ...
        assert!(rename_file(copied_file, "renamed.txt".to_string())
            .await
            .is_ok());

        // 5. 移動操作（subfolder/renamed.txt -> base_path/renamed.txt）
        let renamed_file = format!("{}/renamed.txt", sub_dir);
        assert!(move_files(vec![renamed_file], base_path.clone())
            .await
            .is_ok());
    ...
    async fn test_list_files_sorted_dirs_first() {
        ...
        assert!(
            size_entries[0].is_dir,
            "First entry (size sort) should be a directory"
        );
        assert!(
            size_entries[1].is_dir,
            "Second entry (size sort) should be a directory"
        );
        ...
        assert!(
            modified_entries[0].is_dir,
            "First entry (modified sort) should be a directory"
        );
        assert!(
            modified_entries[1].is_dir,
            "Second entry (modified sort) should be a directory"
        );
