...
pub fn get_icon_binary(id: &str) -> Option<Vec<u8>> {
    ...
    if let Some(cache_dir) = CACHE_DIR.get() {
        let cache_file = cache_dir.join(format!("{}.png", hash_id(id))); // PNGとして扱う
    ...
    use cocoa::foundation::{NSString, NSSize, NSRect, NSPoint};
    ...
    let png_data = unsafe {
        ...
        let result = (|| -> Option<Vec<u8>> {
            ...
            let icon: cocoa_id = if id.starts_with("ext:") {
                let ns_ext = NSString::alloc(nil).init_str(&id[4..]);
                ...
                let path = if id.starts_with("app:") { &id[4..] } else { &id[5..] };
            ...
            if bitmap_rep == nil { return None; }

            // PNGに変換 (NSBitmapImageFileTypePNG = 4)
...
pub async fn list_files_sorted(
    ...
    // app: アイコンを並列プリウォーム（キャッシュミス分のみ NSWorkspace API を叩く）
