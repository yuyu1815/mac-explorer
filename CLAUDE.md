...
| アーカイブ | libarchive2 (Rust) | ZIP/TAR/7z等の圧縮・展開 |

### プラットフォーム対応

**本プロジェクトはmacOSのみを対象としています。**

- WindowsやLinux向けの互換性コードは不要
- パス区切り文字は `/` のみ使用（`\` は考慮不要）
- アーカイブ機能もlibarchive2がmacOSでサポートする形式のみ実装

### アーカイブ機能 (ZIP/圧縮・展開)

**対応形式** (macOSのlibarchive2がサポートする形式のみ):
- ZIP, 7z, TAR, TAR.GZ, TAR.BZ2, TAR.XZ, TAR.ZST

**非対応形式** (検出のみ可能だが圧縮・展開はエラー):
- RAR, ISO, CAB等のプロプライエタリ形式

**セキュリティ対策**:
- パストラバーサル攻撃の検出と拒否 (`../` や絶対パスを含むエントリをスキップ)
- 展開先ディレクトリ外への書き込み防止

**エラーハンドリング**:
- ファイル単位のエラーを収集してユーザーに通知
- 圧縮/展開は継続実行し、完了時にエラー一覧を表示
