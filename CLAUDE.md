...
> **Prompt Template:**
> 「以下の既存コードから達成すべきビジネスロジックを抽出してください。その後、既存の構造・変数名・後方互換性は一切無視し、そのロジックを満たす最もシンプルでモダンなコードをゼロベースで再実装してください。」
...
> **Prompt Template:**
> 「以下の制約を絶対に厳守してコードを生成してください。古いバージョンのためのフラグ分岐 (if legacy: 等) は全て削除すること。関数のネスト（インデント）は最大1段階までとすること。1つの関数の行数は最大15行とすること。」
...
> **Prompt Template:**
> 「この処理をリファクタリングします。まだ実装コードは書かないでください。まず、最も査読性が高くなる理想的な『関数名・引数の型・戻り値の型（インターフェース）』のみを提案してください。私が承認した後に実装に進みます。」
...
> **Prompt Template:**
> 「以下のテストケース（入力と期待される出力）をすべてパスする関数を作成してください。既存システムとの連携は考えず、このテストを通るための最短かつ最もパフォーマンスの良い純粋関数を記述してください。」
